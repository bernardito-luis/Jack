...
import random
# procedures, functions and so on to geberate maps
# =====================================================================================================================
...
def add_treasures(island):
    candidate = 0
    treasures = ['3gn', '5cc', '4cc', '4cc', '3cc', '3cc', '3cc', '2cc', '2cc',
                 '2cc', '2cc', '2cc', '1cc', '1cc', '1cc', '1cc', '1cc']

    while treasures:
        while island[candidate] != '_':
...
def calculate_distances(island_o, possible_maps):
    #dbg
    dists = []
    island = island_o[:]
    add_treasures(island)
    handicap = [0,0,0,0]
    treasures_map = []
    for place in island:
    ...
    def get(self):
        ...
        if zero_record_exists:
            ...
            for i in range(25000):
                calculate_distances(pseudo, possible_maps)            
            ...
            # try:
                # map.info = possible_maps[0]
            # except (IndexError):
                # map.info = "Calculation failed =("
            #map_got[0].put()
            # add new map    
            
            #map.add_map()
            
        # # set head record only once
        # init_db()
        # # create new map
        # map = TreasureMap(parent=map_key())
        # # see the head record
